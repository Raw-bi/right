...
import {
  a as x
} from "./chunk-KSIIUMTG.js";
import {
  d as T
} from "./chunk-YTGE2KHB.js";
...
// src/utils/config.ts
var $ = 5 / 2;
var ee = "https://assets.bwbx.io/font-service/css/BWHaasGroteskWebDingbat:400%7CAvenirNextMForBBG:400,400i,600,600i%7CBWHaasGrotesk:400,500%7CBloomberg-PropU_N:400,600%7CBloomberg-PropU_N-Bold:700/styled-font-face.css?cacheBust";
...
var k = class {
  ...
  async setup() {
    console.log(`ToasterChart rendered via Pantry - ${x}`);
    const e = M(this.data, {
      env: this.tier,
    ...
    this.pkg = n;
    this.mod = this.pkg.legacy ? await g(r) : await g(r);
    this.prepareHTML();
    this.addListeners();
    setTimeout(() => {
      this.handleSizing();
  ...
  prepareHTML = () => {
    ...
    if (this.options.padding) {
      this.element.style.padding = "20px 14px 12px 14px";
    } else {
  ...
  handleSizing = () => {
    ...
    u({ event: "resize", height: r, id: this.id });
  };
...
function y({
  ...
  if (e && e === "dark") {
    s = true;
  } else if (!e && (a.includes("-dark") || h.includes(i))) {
    s = true;
  }
  if (i === "terminal") {
    if (r) {
  ...
  if (r) {
    return {
      theme: d.includes(i) ? i : "terminal",
      legacyTheme: d.includes(i) ? h.includes(i) ? i : `${i}-dark` : "terminal",
      terminalTheme: d.includes(i) ? i : "terminal",
      legacyTerminalTheme: d.includes(i) ? h.includes(i) ? i : `${i}-dark` : "terminal",
      variant: "dark"
    };
  }
  if (h.includes(i)) {
    return {
      ...
      legacyTheme: i,
      terminalTheme: d.includes(i) ? i : "terminal",
      legacyTerminalTheme: d.includes(i) ? i : "terminal",
      variant: "dark"
    };
  }
  if (!q.includes(i)) {
  ...
  return {
    ...
    legacyTerminalTheme: d.includes(i) ? `${i}-dark` : "terminal",
    variant: s ? "dark" : "light"
  };
}
...
function M(t, e) {
  ...
  if (r) {
    ...
    if (o.has("envOverride")) {
      n = o.get("envOverride");
    }
  }
  const a = N(t.version);
  const i = B;
  let s = T(t.type ?? t.chartType, {
  ...
  let l;
  if (!s && t.customRenderImportPath) {
    l = `${j}/${t.customRenderImportPath}`;
  } else if (s) {
    if (e?.assetPath === "/api/pantry-proxy") {
      l = `${s.path}`;
    } else {
      l = `${s.path}?toasterVersion=${i}`;
    }
  } else {
    return null;
  }
  return { importPkg: s, importPath: l };
}
function Ie(t = "terminal", e = "dark") {
...
function Ce(t) {
  ...
  const l = t.animationParams;
  const o = t.preventMobileFont;
  const p = t.forceThemeOverride;
  const O = t.forceLegacyRender;
  const E = t.isTerminalClient;
  const P = t.interactive ?? true;
  const R = true;
  const S = {
    web: R,
    variant: e,
    ...
    ...p && { forceThemeOverride: p },
    ...O && { forceLegacyRender: "true" },
    ...E && { isTerminalClient: E },
  ...
  return new URLSearchParams(
    Object.fromEntries(
      Object.entries(S).map(([L, U]) => [L, String(U)])
    )
  ).toString();
}
function De(t) {
...
async function c({
  ...
  const { onLoad: a } = n;
  let i = W({ pkg: r, theme: e });
  const s = document.getElementById(`toaster-stylesheet-${t}`);
  if (s) {
    s.parentElement?.removeChild(s);
  }
  const l = document.querySelectorAll("link");
  if (Array.from(l).some(
    (p) => p.getAttribute("href") === i
  )) {
