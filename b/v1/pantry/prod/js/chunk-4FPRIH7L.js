import {
  a as g
} from "./chunk-XQ67GXHG.js";
import {
  a as p
} from "./chunk-ADMZL3B2.js";
import {
  d as x
} from "./chunk-VVQXEFPV.js";

// ../../node_modules/nanoid/url-alphabet/index.js
var I = "useandom-26T198340PX75pxJACKVERYMINDBUSHWOLF_GQZbfghjklqvwyzrict";

// ../../node_modules/nanoid/index.browser.js
var B = (i = 21) => {
  let e = "";
  let t = crypto.getRandomValues(new Uint8Array(i |= 0));
  while (i--) {
    e += I[t[i] & 63];
  }
  return e;
};

// src/utils/styles.ts
function U(i, e) {
  let t;
  let r = 0;
  function n() {
    if (Boolean(i.sheet)) {
      cancelAnimationFrame(t);
      e(true);
    } else if (r > 200) {
      cancelAnimationFrame(t);
      console.error(
        "Stylesheet failed to load within the expected time.",
        i.getAttribute("href")
      );
      e(false);
    } else {
      r += 1;
      t = requestAnimationFrame(n);
    }
  }
  t = requestAnimationFrame(n);
}
async function C(i, e) {
  return new Promise((t, r) => {
    U(i, (n) => {
      if (n && e) e();
      return t(n);
    });
  });
}

// src/utils/config.ts
var Z = 5 / 2;
var D = "data:image/png;base64,";

// src/utils/messenger.ts
function _(i, e) {
  i.contentWindow?.postMessage(e, "*");
}
function u(i) {
  window.parent.postMessage(i, "*");
}

// src/utils/sizing.ts
function w(i, e = true) {
  if (!i) return null;
  const t = i.offsetHeight;
  if (!e) return t;
  try {
    const r = globalThis.getComputedStyle(i);
    let n = parseInt(r.marginTop);
    let a = parseInt(r.marginBottom);
    if (Number.isNaN(n)) {
      n = 0;
    }
    if (Number.isNaN(a)) {
      a = 0;
    }
    const s = n + a;
    return t + s;
  } catch {
    return t;
  }
}

// src/ToasterChart.ts
var A = "business";
var V = 500;
var K = 500;
var G = "toaster-chart";
var b = class {
  cachedHeight;
  resizeCheckCount;
  loaded;
  element;
  data;
  options;
  id;
  type;
  error;
  instance;
  mod;
  tier;
  sizeInterval;
  pkg;
  constructor(e, t) {
    this.element = e;
    this.sizeInterval = 0;
    this.cachedHeight = 0;
    this.resizeCheckCount = 0;
    this.loaded = false;
    this.tier = t.tier ?? "prod";
    this.pkg = null;
    this.mod = null;
    this.instance = null;
    this.error = "";
    this.data = y(t.data);
    if (!t.data) {
      throw new Error("No data supplied to chart");
    }
    this.id = t?.id ?? this.data.id ?? M();
    this.type = this.data.type || this.data.chartType;
    this.options = {
      variant: "light",
      // option to determine whether or not
      // to dynamically fetch + inject the css
      // needed for legacy charts into the document.head
      // this will have no effect for contemporary "pantry"
      // charts, but will only run for legacy toaster-client charts
      //
      // this should be set to false if the themed CSS file
      // for a toaster chart is already on the page
      injectStyles: true,
      ...t
    };
    if (!this.type) {
      console.error("Chart has no type specified in data");
      this.prepareFallback();
      return;
    }
    this.setup();
  }
  /**
   *
   * @returns
   */
  get theme() {
    const e = this.data;
    const t = this.id;
    const r = E({
      requestedTheme: k(e),
      isTerminalClient: this.options?.isTerminalClient,
      variant: m(this.options.variant),
      themeOverride: this.options.forceThemeOverride
    });
    if (typeof window !== "undefined") {
      const n = `_toaster_debug_${t}`;
      window[n] = function() {
        return { theme: r, data: e };
      };
    }
    return r;
  }
  async setup() {
    console.log(`ToasterChart rendered via Pantry - ${g}`);
    const e = Q(this.data, {
      env: this.tier,
      assetPath: this.options?.assetPath,
      useAbsolutePaths: this.options?.useAbsolutePaths
    });
    if (!e) {
      const n = `Unable to resolve pkg for ${this.data.chartType || this.data.type}`;
      console.error(n);
      return;
    }
    const { importPath: t, importPkg: r } = e;
    this.pkg = r;
    this.mod = this.pkg.legacy ? await p(t) : await p(t);
    this.prepareHTML();
    this.addListeners();
    setTimeout(() => {
      this.handleSizing();
    }, K);
    if (this.pkg.legacy) {
      if (this.options.injectStyles) {
        this.updateRootClasses();
        await d({
          id: this.id,
          pkg: this.pkg,
          options: this.options,
          theme: this.theme.legacyTheme
        });
      }
      return this._legacyRender({
        element: this.element,
        // legacy chart renders use the theme from data
        // so overwrite here if possible
        data: { ...this.data, theme: this.theme.legacyTheme },
        options: {
          ...this.options,
          // this is important to get scatterplot scales
          // to show up w correct dark mode
          ...this.theme.variant === "dark" && { darkTheme: true },
          // pass original/deprecated theme
          // to options for svelte scales
          theme: this.theme.legacyTheme
        },
        mod: this.mod
      });
    }
    this.render();
    if (this.options.onLoad) this.options.onLoad();
  }
  updateRootClasses() {
    const e = this.element;
    e.classList.forEach((t) => {
      if (t.startsWith("toaster-theme")) {
        e.classList.remove(t);
      }
    });
    e.classList.remove("toaster-variant-dark");
    e.classList.remove("toaster-variant-light");
    e.classList.remove("toaster-transparent-bg");
    if (this.options.isTerminalClient) {
      e.classList.add("toaster-is-terminal");
    }
    if (this.pkg?.legacy) {
      if (this.options.isTerminalClient) {
        e.setAttribute(
          "data-toaster-theme",
          this.theme.legacyTerminalTheme
        );
        e.classList.add(
          `toaster-theme-${this.theme.legacyTerminalTheme}`
        );
      } else {
        e.setAttribute("data-toaster-theme", this.theme.legacyTheme);
        e.classList.add(`toaster-theme-${this.theme.legacyTheme}`);
      }
    } else {
      if (this.options.isTerminalClient) {
        e.setAttribute("data-toaster-theme", this.theme.terminalTheme);
        e.classList.add(`toaster-theme-${this.theme.terminalTheme}`);
      } else {
        e.setAttribute("data-toaster-theme", this.theme.theme);
        e.classList.add(`toaster-theme-${this.theme.theme}`);
      }
    }
    if (this.theme.variant === "dark") {
      e.classList.add(`toaster-variant-dark`);
    } else {
      e.classList.add("toaster-variant-light");
    }
    if (this.options.transparency === true) {
      e.classList.add("toaster-transparent-bg");
    }
  }
  addListeners() {
    let e = this;
    window.addEventListener("load", async () => {
      this.loaded = true;
      document.documentElement.classList.add("loaded");
      u({ event: "chart-loaded", id: this.id });
    });
    let t = window.matchMedia("(prefers-color-scheme: dark)");
    t.addEventListener("change", async () => {
      if (e.options.variant === "auto") {
        e.destroy();
        e.updateRootClasses();
        const r = m("auto");
        if (e.pkg?.legacy) {
          await d({
            id: this.id,
            pkg: this.pkg,
            options: this.options,
            theme: this.theme.legacyTheme
          });
          const n = {
            element: e.element,
            // legacy chart renders use the theme from data
            // so overwrite here if possible
            data: { ...e.data, theme: e.theme.legacyTheme },
            options: {
              ...e.options,
              // this is important to get scatterplot scales
              // to show up w correct dark mode
              ...r === "dark" && { darkTheme: true },
              // pass original/deprecated theme
              // to options for svelte scales
              theme: this.theme.legacyTheme,
              variant: r
            },
            mod: this.mod
          };
          e._legacyRender(n);
        } else {
          await e.render();
        }
      }
    });
    globalThis.addEventListener(
      "message",
      async (r) => {
        let n = this.pkg?.legacy && this.options.injectStyles;
        if (r.data.id !== this.id) return;
        if (r.data.event === "rerender") {
          if (r.data.props.variant) {
            this.options.variant = r.data.props.variant;
          }
          if (r.data.props.theme) {
            this.updateRootClasses();
            if (n) {
              await d({
                pkg: this.pkg,
                options: this.options,
                id: this.id,
                theme: this.theme.legacyTheme
              });
            }
          }
          if (this.pkg?.legacy) {
            this.destroy();
            this.setup();
          }
        }
      }
    );
    document.fonts.ready.then(() => {
      window.dispatchEvent(new Event("load"));
    });
  }
  prepareFallback = () => {
    this.element.classList.add("error");
    this.element.style.display = "flex";
    this.element.style.alignItems = "center";
    this.element.style.flexDirection = "column";
    this.element.style.justifyContent = "center";
    this.element.style.width = "100%";
    this.element.style.height = "200px";
    const e = document.createElement("img");
    e.width = 96;
    e.height = 96;
    e.src = D;
    const t = document.createElement("p");
    t.style.fontFamily = "sans-serif";
    t.textContent = "Untyped Toaster chart";
    this.element.appendChild(e);
    this.element.appendChild(t);
  };
  prepareHTML = () => {
    const e = `toaster-type-${this.type}`;
    this.element.setAttribute(`data-chart-id`, this.id);
    this.element.id = `toaster-id-${this.id}`;
    this.element.classList.add(G, e);
    this.updateRootClasses();
    if (this.options.padding) {
      this.element.style.padding = "20px 14px 12px";
    } else {
      this.element.style.padding = "0px";
    }
    if (this.options.transparency === true) {
      if (this.element.parentElement) {
        this.element.parentElement.style.backgroundColor = "transparent";
      } else {
        this.element.style.backgroundColor = "transparent";
      }
    }
  };
  // Check size on a regular interval,
  // and broadcast out if any change has occurred
  // this is a pretty simple setup and avoids the
  // any possibility of an infinite loop
  handleSizing = () => {
    const e = this.element;
    this.sizeInterval = globalThis.setTimeout(
      this.handleSizing,
      V
    );
    let t = w(e) ?? 0;
    this.resizeCheckCount += 1;
    if (Math.round(this.cachedHeight) === Math.round(t)) {
      return;
    }
    if (t > 0) {
      e.setAttribute("data-height", `${t}`);
    }
    this.cachedHeight = t;
    if (this.options.onResize) {
      this.options.onResize(t);
    }
    u({ event: "resize", height: t, id: this.id });
  };
  // "New" pantry chart render function
  render = async () => {
    this.instance = await this.mod.render({
      element: this.element,
      options: {
        ...this.options,
        variant: m(this.options.variant)
      },
      // we want to pass any explicit theme
      // onto the chartData
      chartData: {
        ...this.data,
        // legacy schema...
        theme: this.theme,
        // new schema...
        design: {
          ...this.data.design || {},
          originalTheme: this.theme.theme
        }
      }
    });
    document.fonts.ready.then(() => {
      window.dispatchEvent(new Event("load"));
    });
  };
  // Re-render with updated props
  // Right now, this only allows updating
  // theme/variant in one go, but can be expanded
  // to allow live updating data
  rerender = async (e) => {
    const { data: t, variant: r, isTerminalClient: n } = e;
    if (n === true || n === false) {
      this.options.isTerminalClient = n;
    }
    this.data = { ...this.data, ...t };
    if (r) {
      this.options.variant = r ?? this.theme.variant;
      this.updateRootClasses();
      if (this.pkg?.legacy && this.options.injectStyles) {
        await d({
          pkg: this.pkg,
          options: this.options,
          id: this.id,
          theme: this.theme.legacyTheme
        });
      }
    }
    this.instance?.rerender({
      ...t && { chartData: y(t) },
      ...r && { variant: r }
    });
  };
  _legacyRender({
    element: e,
    data: t,
    options: r,
    mod: n
  }) {
    return n.factory(e, t, r);
  }
  /**
   * Destroy method. Called from consumer side...
   */
  destroy = () => {
    this.instance?.destroy();
    window.clearTimeout(this.sizeInterval);
  };
};

// src/utils/theme.ts
var z = [
  "bi",
  "bnef",
  "business",
  "opinion",
  // we don't allow choosing terminal
  // in the ui, but we do allow it via
  // query param
  "terminal"
];
function E({
  requestedTheme: i,
  variant: e,
  isTerminalClient: t,
  themeOverride: r
}) {
  let n = r ?? i;
  const a = n.replace("-dark", "");
  let s = t ?? false;
  if (e && e === "dark") {
    s = true;
  } else if (!e && (n.includes("-dark") || T.includes(a))) {
    s = true;
  }
  if (a === "terminal") {
    if (t) {
      return {
        theme: "terminal",
        legacyTheme: "terminal",
        terminalTheme: "terminal",
        legacyTerminalTheme: "terminal",
        variant: "dark"
      };
    }
    return {
      theme: A,
      legacyTheme: `${A}${s ? "-dark" : ""}`,
      terminalTheme: "terminal",
      legacyTerminalTheme: "terminal",
      variant: s ? "dark" : "light"
    };
  }
  if (a === "bi") {
    if (t) {
      return {
        theme: "bi",
        legacyTheme: "bi",
        terminalTheme: "bi",
        legacyTerminalTheme: "bi",
        variant: "dark"
      };
    }
    return {
      theme: "bi",
      legacyTheme: "bi",
      terminalTheme: "bi",
      legacyTerminalTheme: "bi",
      variant: s ? "dark" : "light"
    };
  }
  if (t) {
    return {
      theme: l.includes(a) ? a : "terminal",
      legacyTheme: l.includes(a) ? f.includes(a) ? a : `${a}-dark` : "terminal",
      terminalTheme: l.includes(a) ? a : "terminal",
      legacyTerminalTheme: l.includes(a) ? f.includes(a) ? a : `${a}-dark` : "terminal",
      variant: "dark"
    };
  }
  if (T.includes(a)) {
    return {
      theme: a,
      legacyTheme: a,
      terminalTheme: l.includes(a) ? a : "terminal",
      legacyTerminalTheme: l.includes(a) ? a : "terminal",
      variant: "dark"
    };
  }
  if (!z.includes(a)) {
    return {
      theme: A,
      legacyTheme: `${A}${s ? "-dark" : ""}`,
      terminalTheme: "terminal",
      legacyTerminalTheme: "terminal",
      variant: s ? "dark" : "light"
    };
  }
  return {
    theme: a,
    legacyTheme: `${a}${s ? "-dark" : ""}`,
    terminalTheme: l.includes(a) ? a : "terminal",
    // should always be dark
    legacyTerminalTheme: l.includes(a) ? `${a}-dark` : "terminal",
    variant: s ? "dark" : "light"
  };
}

// src/utils/index.ts
var l = [
  "bi",
  "bnef",
  "beco",
  "opinion",
  "terminal"
];
var T = ["terminal"];
var f = ["terminal", "bi"];
var Te = "toaster-theme-css";
var q = "https://www.bloomberg.com/toaster-packages/v1/custom";
function y(i) {
  if (typeof i === "string") {
    return JSON.parse(i);
  }
  return i;
}
function M() {
  return B();
}
function j(i) {
  if (!i) return 0;
  const [e] = i.split(".");
  if (!e) return 0;
  const t = parseInt(e, 10);
  if (Number.isNaN(t)) return 0;
  return t;
}
function Q(i, e) {
  const t = typeof window !== "undefined";
  let r = e?.env ?? "prod";
  if (t) {
    let h = new URLSearchParams(window.location.search);
    if (h.has("envOverride")) {
      r = h.get("envOverride");
    }
  }
  const n = j(i.version);
  let a = x(i.type ?? i.chartType, {
    assetPath: e?.assetPath,
    env: r,
    version: n,
    useAbsolutePaths: Boolean(e?.useAbsolutePaths),
    terminal: Boolean(e?.terminal)
  });
  let s;
  if (!a && i.customRenderImportPath) {
    s = `${q}/${i.customRenderImportPath}`;
  } else if (a) {
    if (e?.assetPath === "/api/proxy/pantry") {
      s = `${a.path}`;
    } else {
      s = `${a.path}?toasterVersion=${g}`;
    }
  } else {
    return null;
  }
  return { importPkg: a, importPath: s };
}
function m(i) {
  if (!i) return "light";
  if (i === "light" || i === "dark") return i;
  if (i === "auto" && typeof window !== "undefined") {
    try {
      let e = window.matchMedia("(prefers-color-scheme: dark)").matches ? "dark" : "light";
      return e;
    } catch {
      console.error("Unable to get auto color scheme");
      return "light";
    }
  }
  return "light";
}
function fe(i) {
  const e = m(i.variant);
  const t = i.titles ?? true;
  const r = i.footer ?? true;
  const n = i.logo ?? true;
  const a = i.transparency ?? false;
  const s = i.padding ?? false;
  const h = i.animationParams;
  const o = i.preventMobileFont;
  const c = i.forceThemeOverride;
  const O = i.forceLegacyRender;
  const v = i.isTerminalClient;
  const P = i.interactive ?? true;
  const S = true;
  const R = {
    web: S,
    variant: e,
    logo: n,
    titles: t,
    interactive: P,
    padding: s,
    footer: r,
    hideLogo: !n,
    ...a === true && { transparency: a },
    ...c && { forceThemeOverride: c },
    ...O && { forceLegacyRender: "true" },
    ...v && { isTerminalClient: v },
    // add legacy query params for legacy charts
    // NOTE: There is a bug in toaster-client
    // that simply checks existence of hideTitles
    // and will affect padding. This means
    // we only send hideTitles param if explicitly true
    ...t === false && { hideTitles: true },
    // same goes for noPadding
    ...s === false && { noPadding: true },
    ...h && { ...h },
    ...o && { preventMobileFont: o }
  };
  return new URLSearchParams(
    Object.fromEntries(
      Object.entries(R).map(([L, H]) => [L, String(H)])
    )
  ).toString();
}
function ve(i) {
  return new Promise((e) => {
    setTimeout(e, i);
  });
}
function Y({
  pkg: i,
  theme: e
}) {
  const t = i.path;
  const [r, n] = t.split("/js/");
  let a = `${r}/css/${e}.css`;
  return a;
}
async function d({
  id: i,
  theme: e,
  pkg: t,
  options: r
}) {
  const { onLoad: n } = r;
  let a = Y({ pkg: t, theme: e });
  const s = document.getElementById(`toaster-stylesheet-${i}`);
  if (s) {
    s.parentElement?.removeChild(s);
  }
  const h = document.querySelectorAll("link");
  if (Array.from(h).some(
    (c) => c.getAttribute("href") === a
  )) {
    return Promise.resolve();
  }
  const o = document.createElement("link");
  o.href = a;
  o.type = "text/css";
  o.rel = "stylesheet";
  o.id = `toaster-stylesheet-${i}`;
  o.setAttribute("data-testid", "legacy-stylesheet");
  if (r.injectionElement) {
    r.injectionElement.appendChild(o);
  } else {
    document.head.appendChild(o);
  }
  await C(o, n);
}
function xe(i) {
  const e = /\/charts\/([^/]+?)(?:\.html|\?|$)/;
  const t = i.match(e);
  return t ? t[1] : null;
}
function k(i) {
  const e = (
    // prefer explicit options passed in
    // otherwise, look at new chart schema
    (i.design?.originalTheme ?? // otherwise, look at legacy chart schema
    i.theme ?? i.webTheme) || // otherwise, just use business
    "business"
  );
  return e;
}

export {
  B as a,
  E as b,
  l as c,
  T as d,
  f as e,
  Te as f,
  q as g,
  y as h,
  M as i,
  j,
  Q as k,
  m as l,
  fe as m,
  ve as n,
  Y as o,
  d as p,
  xe as q,
  k as r,
  _ as s,
  w as t,
  b as u
};
