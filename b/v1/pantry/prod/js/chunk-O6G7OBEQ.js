import {
  a as g
} from "./chunk-OUYRFPHX.js";
import {
  a as p
} from "./chunk-TRZ7WIL2.js";
import {
  d as I
} from "./chunk-E7XEBGQ5.js";
import {
  a as x
...
function D(i, e = true) {
  ...
  if (!e) return t;
  try {
    const n = globalThis.getComputedStyle(i);
    let a = parseInt(n.marginTop);
    let r = parseInt(n.marginBottom);
    if (Number.isNaN(a)) {
      a = 0;
    }
    if (Number.isNaN(r)) {
...
var b = class {
  ...
  get theme() {
    ...
    const n = E({
      ...
      variant: A(this.options.variant),
      themeOverride: this.options.forceThemeOverride
    });
    if (typeof window !== "undefined") {
      const a = `_toaster_debug_${t}`;
      window[a] = function() {
  ...
  async setup() {
    ...
    const { importPath: t, importPkg: n } = e;
    this.pkg = n;
  ...
  addListeners() {
    ...
    function t() {
      ...
      u({ event: "chart-loaded", id: e.id });
    }
    window.addEventListener("load", t);
    window.addEventListener("toaster-load", t);
    ...
    globalThis.addEventListener(
      "message",
      async (a) => {
        let r = this.pkg?.legacy && this.options.injectStyles;
        if (a.data.id !== this.id) return;
        if (a.data.event === "rerender") {
          if (a.data.props.variant) {
            this.options.variant = a.data.props.variant;
          }
          if (a.data.props.theme) {
            this.updateRootClasses();
            if (r) {
              await d({
                pkg: this.pkg,
    ...
    document.fonts.ready.then(() => {
      window.dispatchEvent(new CustomEvent("toaster-load"));
    });
  }
  prepareFallback = () => {
  ...
  prepareHTML = () => {
    ...
    this.element.classList.add(z, e);
    this.updateRootClasses();
  ...
  rerender = async (e) => {
    const { data: t, variant: n, isTerminalClient: a, isEditorDesignTab: r } = e;
    if (a === true || a === false) {
      this.options.isTerminalClient = a;
    }
    if (r === true || r === false) {
      this.options.isEditorDesignTab = r;
    }
...
function E({
  ...
  let a = n ?? i;
  const r = a.replace("-dark", "");
  let s = t ?? false;
  if (e && e === "dark") {
    s = true;
  } else if (!e && (a.includes("-dark") || T.includes(r))) {
    s = true;
  }
  if (r === "terminal") {
    if (t) {
  ...
  if (t) {
    return {
      theme: l.includes(r) ? r : "terminal",
      legacyTheme: l.includes(r) ? y.includes(r) ? r : `${r}-dark` : "terminal",
      terminalTheme: l.includes(r) ? r : "terminal",
      legacyTerminalTheme: l.includes(r) ? y.includes(r) ? r : `${r}-dark` : "terminal",
      variant: "dark"
    };
  }
  if (T.includes(r)) {
    return {
      ...
      legacyTheme: r,
      terminalTheme: l.includes(r) ? r : "terminal",
      legacyTerminalTheme: l.includes(r) ? r : "terminal",
      variant: "dark"
    };
  }
  if (!G.includes(r)) {
  ...
  return {
    theme: r,
    legacyTheme: `${r}${s ? "-dark" : ""}`,
    terminalTheme: l.includes(r) ? r : "terminal",
    // should always be dark
    legacyTerminalTheme: l.includes(r) ? `${r}-dark` : "terminal",
    variant: s ? "dark" : "light"
  };
}
...
function M(i, e) {
  ...
  let s;
  if (!r && i.customRenderImportPath) {
    s = `${q}/${i.customRenderImportPath}`;
  } else if (r) {
    if (e?.assetPath === "/api/proxy/pantry") {
      s = `${r.path}`;
    } else {
      s = `${r.path}?toasterVersion=${g}`;
    }
  } else {
    return null;
  }
  return { importPkg: r, importPath: s };
}
function A(i) {
  if (!i) return "light";
...
function fe(i) {
  ...
  const P = {
    ...
    footer: n,
    hideLogo: !a,
    ...r === true && { transparency: r },
    ...c && { forceThemeOverride: c },
    ...k && { forceLegacyRender: "true" },
  ...
  return new URLSearchParams(
    Object.fromEntries(
      Object.entries(P).map(([R, L]) => [R, String(L)])
    )
  ).toString();
}
function Y({
...
//# sourceMappingURL=chunk-O6G7OBEQ.js.map
